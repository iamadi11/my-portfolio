...
# PATH fix: prefer the native platform binary (no node required) over the npm wrapper.
_FALLOW_NATIVE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && cd ../.. && pwd)/node_modules/@fallow-cli/darwin-arm64"
[ -d "$_FALLOW_NATIVE" ] && export PATH="$_FALLOW_NATIVE:$PATH"
unset _FALLOW_NATIVE
# Installer version: 2.89.0
